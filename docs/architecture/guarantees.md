...
| Data in the cache before the screen renders, and preloaded on intent             | the router's loader, which a builder writes and a composed screen exports    |
...
| Every state drawn: loading, empty, error, forbidden and offline                  | the builders, the components a composed screen uses, and the standard plugin |
...
| One list pattern and one form pattern across every plugin                        | the shared components, and the catalogue that documents them                 |
