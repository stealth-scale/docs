...
description: The four files of a component, where a prop is described, what a story file proves, what belongs in the spec, what the page contains, and what the catalogue enforces.
sidebar:
  order: 7
---
...
The catalogue's extractor builds the props table from the component's own docblocks, and a
story file writes no `argTypes` for documentation, because a description written there drifts
away from the signature it describes. Every prop in the table is declared on the component's own
...
export interface ButtonProps extends ButtonPrimitive.Props {
   ...
   * so they have no accessible name until `aria-label` gives them one.
   *
   * @category Appearance
   * @default default
   */
...
Every prop that can be driven from the panel is driven from it, and the Playground, as the
page's first story and the one the controls point at, seeds the panel with its args. The
extractor decides the control from the type:
...
under a comment saying which domain or which edge it exists to show; the meta; the
Playground; the examples.
...
- The first export is `Playground`. Every other story spreads `example` from the catalogue's
  ...
  without evaluating it and a tag reached through a spread never arrives. The sidebar then
  shows exactly two entries, Docs and Playground.
- Fixtures move to `<name>.fixtures.ts` once they take up more room than the scenes: more than
  about sixty lines before the first story, or a story file past three hundred lines. The
  fixtures file
...
Every example draws a different domain, and the house domains are payments and billing, healthcare,
...
- Values are realistic: names from more than one locale, amounts with a currency and a
  ...
  the zero-byte file, and the fixture comment says which edge it is.
- The Playground's fixture is the home domain: the component's most typical production use,
  fully populated, because it is also the example a reader edits in the panel.
...
cannot be missing from the page. Where a provider renders no element it has no stories, its
effect is measured in the stories of what it affects, and its page says where.

The RTL story measures something the component owns: which side an icon is on, which
gutter a check mark moves to, whether a rail is at the right edge, that a plot keeps its
direction while its caption flips. `direction === 'rtl'` on the root proves only that the
harness works. Where nothing about the component mirrors, the story says so and measures what
does not move. An LTR and RTL pair is the one sanctioned shared render, because the
...
A reader recreates the usage from each story's real source, so state wiring is written inline
in each story's `render`, in a named function, with handlers composing `args.onX?.()` so the
Actions panel still records every call. The duplication across stories is deliberate,
and hoisted wiring harnesses are not used. Shared story-local components exist only for what a consumer
...
- Every `expect` is awaited: `findBy*` for what mounts async, `waitFor` after a state change
  ...
  scoped to the popup with `data-open`.
- A comment says which failure each assertion catches, and a reader takes it as documentation
  when the test goes red.
- An accessibility rule exemption carries its justification on the parameters object.
- A play that interacts with the component is wrapped in `interactive()` from the kit: the
  catalogue plays every story a docs page mounts, so without the wrapper six sheets open the
  moment the page is opened. A play that only measures what is already on screen is not
  wrapped.

## Specs

The stories cover behaviour and accessibility in a browser, and everything they cannot cover
goes in the spec: the vendoring contract (the slot, the class merge order, the ref, the prop
spread); recipe coverage, with every documented variant producing its own classes; the paths
a story cannot get to, such as an `error` event or a rerender with a new prop; and the
promises the component makes about what it leaves alone, such as the role, the classes and
the announcements it adds to nothing. Every
spec opens with the same header, so a reader knows where the rest of the tests are, and it
never runs a story.
...
- Every non-Playground story has a section that argues for it, and each canvas refers to its
  story by export, so renaming a story renames it on the page too.
...
- A family's parts are one `## Every part` section with a bold lead-in per part and its
  `<ArgTypes>`; never a `###` per part, which pushes the behaviours out of the table of
  contents.
...
own `node_modules`, so an undeclared block import resolves nowhere. Only the catalogue build
reports that.
...
## Behaviour set once in the preview

The preview sets all of this, so no story has to:
